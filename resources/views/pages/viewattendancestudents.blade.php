    ...
    <section class="content">
        ...
        <div class="card card-default">
          ...
          <div class="card-body">
                ...
                <form id="addschool" method="POST" action="/shoolreg" enctype="multipart/form-data">
                    @csrf
                        ...
                        <select id="studentclass" name="studentclass" style="border: none; background-color:#EEF0F0;" class="form-control form-control-lg" type="text" placeholder="School Name">
                            ...
                            <option value="{{$classlist->id}}" {{ count($studentDetails) > 6 ? $studentDetails['addteachers'][0]['classid'] == $classlist->id ? "" : "disabled" : "" }}>{{$classlist->classnamee}}</option>
                          @endforeach
                        </select>
                        </div>
                        <div class="form-group">
            ...
            <!-- /.row -->
            <button id="addschoolentirey" type="button" onclick="getstudentList()" class="btn btn-info" form="addschool">Get list</button>
...
// -------------------------------------------------------------------------------------
//        fetch teacher for selected class and check if attendance has been taken
// -------------------------------------------------------------------------------------
    ...
    function getstudentList() {
      ...
      var datehist = document.getElementById('todaysdate').value;
    ...
    var datecheck = /^[0-9-]+$/; 
      ...
      if (studentclassMain == "") {
        toastError('Invalid class')
      }else if (studentsectionMain == "") {
        toastError('Invalid section')
      }else if (studentshiftMain == "") {
        toastError('Invalid shift')
      }else if(studentSession == ""){
        toastError('Invalid session')
      }else if (datehist == "") {
        toastError('Invalid date')
      }else if(datehist.length != 10){
        toastError('Invalid date')
      }else if(!datehist.match(datecheck)){
        toastError('Invalid date')
      }else if(!datehist.includes("-")){
        toastError('Invalid date')
      }else{
        var letters = /^[0-9/]+$/; 
      ...
      headers: {
        ...
        $.ajax({
          url:"/viewatstudents", //the page containing php script
          method: "POST", //request type,
          cache: false,
          ...
          success:function(result){
              ...
              for (let index = 0; index < result[0].length; index++) {
                ...
                if (n) {
                    $('#studenttable').find('tbody').append('<tr><td>' + result[0][index]['id'] + '</td><td>' + result[0][index]['firstname'] + ' ' + result[0][index]['middlename'] + ' ' + result[0][index]['lastname'] +'</td><td>'+ result[0][index]['classnamee'] +'</td><td>'+ result[0][index]['studentsection'] +'</td><td>'+ result[0][index]['gender'] +'</td><td><button class="btn"><i style="color: green;" class="fas fa-check"></i></button></td></tr>');
                }else{
                    $('#studenttable').find('tbody').append('<tr><td>'+ result[0][index]['id'] +'</td><td>'+ result[0][index]['firstname'] + ' ' + result[0][index]['middlename'] + ' ' + result[0][index]['lastname'] +'</td><td>'+ result[0][index]['classnamee'] +'</td><td>'+ result[0][index]['studentsection'] +'</td><td>'+ result[0][index]['gender'] +'</td><td><button class="btn"><i style="color: red;" class="fas fa-times"></i></button></td></tr>');
                }
              }
         },
         error:function(){
...
// ----------------------------------------------------------------------------------------
   ...
   function scrollocation(){
        ...
        document.getElementById('viewstudentattendance').className = "nav-link active"
    }
</script>
