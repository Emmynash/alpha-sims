...
class ResultGenerationController extends Controller
{
    ...
    public function bulkResultPrinting(Request $request)
    {
        // $validatedData = $request->validate([
        //     'classid' => 'required',
        //     'term' => 'required',
        ...
        $studentList = Addstudent_sec::join('users', 'users.id','=','addstudent_secs.usernamesystem')->where(['addstudent_secs.classid'=>$classid, 'addstudent_secs.studentsection'=>$section, 'addstudent_secs.schoolsession'=>$request->session])->select('addstudent_secs.*', 'users.firstname', 'users.middlename', 'users.lastname')->get();
        ...
        for ($i=0; $i < count($studentList); $i++) { 
            ...
            $formTeacherComment = $addschool->getStudentComment($studentList[$i]->id, $classid, $term, $schoolsession);
            ...
            $motolistbeha = MotoList::leftjoin('add_moto_secs', 'add_moto_secs.moto_id','=','moto_lists.id')
        ->select("moto_lists.*", 'add_moto_secs.moto_score')
        ->where(['moto_lists.schoolid' => Auth::user()->schoolid, 'moto_lists.category' => 'behaviour', 'add_moto_secs.student_id'=>$studentList[$i]->id, 'add_moto_secs.session'=>$schoolsession, 'add_moto_secs.term'=>$term])->get();

        $motolistSkill = MotoList::leftjoin('add_moto_secs', 'add_moto_secs.moto_id','=','moto_lists.id')
        ->select("moto_lists.*", 'add_moto_secs.moto_score')
        ->where(['moto_lists.schoolid' => Auth::user()->schoolid, 'moto_lists.category' => 'skills', 'add_moto_secs.student_id'=>$studentList[$i]->id, 'add_moto_secs.session'=>$schoolsession, 'add_moto_secs.term'=>$term])->get();
        ...
        foreach ($motolistSkill as $key => $value) {
            ...
            <div class="studentDetails">
                <div class="studentDetailsone">
                    <table style="width: 100%;">
                        <tr>
                            <td>Name of Student:</td>
                            <td>'.$studentList[$i]->firstname." ".$studentList[$i]->middlename." ".$studentList[$i]->lastname.'</td>
                        </tr>
                        <tr>
                            <td>Class:</td>
                            <td>'.$getClass->classname.$getSection->sectionname.'</td>
                ...
                <div class="studentDetailstwo" style="">
                    <table style="width: 100%;">
                        ...
                        <tr>
                            <td>Admission No:</td>
                            <td>'.$studentList[$i]->admission_no.'</td>
                        </tr>
                        <tr>
                            <td>No in Class:</td>
                            <td>'.count($getSection->getClassCount($classid, $schoolsession, $getSection->id)).'</td>
            ...
            <div>
                <table style="width: 100%;">
                    ...
                    <tr>
                        ...
                        <th></th>
                        <th></th>
                        '.implode(" ",$this->getSubjectScores($term, $studentList[$i]->id, $schoolsession, $classid, $section)).'
                    </tr>
            ...
            <br>
            <div style="width: 100%;">
                <i style="font-style:normal; padding: 8px;">Grand Total: '.round($this->getStudentGrandTotal($term, $classid, $section, $studentList[$i]->id, $schoolsession), 2).'</i> 
                <i style="font-style:normal; padding: 8px;">Student/Pupil Average: '.round($this->getStudentAverge($term, $classid, $studentList[$i]->id, $schoolsession), 2).'</i>
                <i style="font-style:normal; padding: 8px;">Class Average: '.round($this->getClassAverge($term, $classid, $schoolsession, $section), 2).'</i>
                <i style="font-style:normal; padding: 8px;">Position: Nill</i>
            </div>
            <br>
            ...
            <div style="width: 100%; margin-bottom: 10px;">
                <p style="padding: 0px; margin: 0;">HEAD OF SCHOOL\'S COMMENT: '.$this->getHeadOfSchoolComment(round($this->getStudentAverge($term, $classid, $studentList[$i]->id, $schoolsession), 2)).'</p>
                <div style="height: 1px; width: 100%; background-color: black;"></div>
            </div>
            <div style="width: 100%; margin-bottom: 8px;">
                <p style="padding: 0px; margin: 0;">HEAD OF SCHOOL\'S SIGNATURE: <img src="'.$addschool->schoolprincipalsignature.'" height="50px"></p>
